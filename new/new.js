...
var Scatterplot = function(){
    ...
    this.createCircles = function(){
        ...
        ;
        //this.select("body").append("svg").selectAll("circle");
        //select from body, where the object is named svg, then select circle
    ...
    this.createRectangles = function(){
        ...
        this.datapoint = this.svgContainer.selectAll("rect")
            ...
            //.attr("cx", function(d){return x;})   d is the data we are currently in
            .attr("y", function(d){
                return _vis.yAxisScale(d["AverageRating"]) - 6 + 3*d["ARL_Serial"];
            })
            .style("fill", function(d){
                return colorScale(d.PrimaryGenre)
            })
    ...
    this.createTriangles = function(){
        this.datapoint = this.svgContainer.selectAll("polygon")
            ...
            .enter()
            .append("polygon")
            /*.attr("points", -function(d){return sin(toRadian(360 / d.ARL_Count / 2))*len;} + "," + -function(d){return cos(toRadian(360 / d.ARL_Count / 2))*len;}
            + " " + function(d){return sin(toRadian(360 / d.ARL_Count / 2))*len;} + "," + -function(d){return cos(toRadian(360 / d.ARL_Count / 2))*len;}
            + " " + function(d){return _vis.xAxis.PrimaryGenreScale(d["LanguagesSupported"]);} + "," + function(d){return _vis.yAxisScale(d["AverageRating"]);})*/
